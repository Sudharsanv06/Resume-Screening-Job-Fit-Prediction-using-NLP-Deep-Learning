...
def get_skill_gaps(resume_text: str, predicted_role: str) -> dict[str, Any]:
    ...
    return {"role": predicted_role, "present": present, "missing": missing, "fit_pct": fit_pct}


def get_alternative_paths(
    all_probs: dict[str, float], predicted_role: str, resume_text: str = ""
) -> list[dict[str, Any]]:
    """
    ...
    for role, score in sorted_probs:
        ...
        alternative_paths.append({"role": role, "score": float(score), "gap_count": int(gap_count)})

    return alternative_paths


def get_resume_dna(
    resume_text: str, all_probs: dict[str, float], predicted_role: str
) -> dict[str, Any]:
    """
